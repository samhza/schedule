...
function hasPrereqs(prereqs, has) {
    ...
    console.log("TODO: " + prereqs.constructor.name);
...
let courses, courseMap, fallCourses, springCourses, fallSet, springSet;
let fresh = false;
try {
    ({ fallCourses, springCourses } = JSON.parse(readFileSync("cached.json")));
    courses = fallCourses.concat(springCourses);
    courseMap = new Map();
    courses.forEach(c=>courseMap.set(c.courseString, c));
    fallSet = new Set(fallCourses.map(c=>c.courseString));
    springSet = new Set(springCourses.map(c=>c.courseString));
} catch(error) {
    console.log(error)
    readFresh();
}

function getCourse(cs) {
    let course = courseMap.get(cs)
    if (course || fresh) return course;
    readFresh();
    return getCourse(cs);
}
...
function readFresh() {
    fresh = true;
    fallCourses = JSON.parse(readFileSync("fall24.json"));
    springCourses = JSON.parse(readFileSync("spring24.json"));
    courses = fallCourses.concat(springCourses);
    courseMap = new Map();
    courses.forEach(c=>courseMap.set(c.courseString, c));
    fallSet = new Set(fallCourses.map(c=>c.courseString));
...
if(fresh){
    const toCache = new Set(csElectives);
    allCourses.forEach(toCache.add, toCache);
    let data = {
	fallCourses: [],
	springCourses: [],
    }
    for(const course of toCache){
	let crs = getCourse(course);
	if(!crs) continue;
	if(fallSet.has(course)){
	    data.fallCourses.push(crs);
	}
	if(springSet.has(course)){
	    data.springCourses.push(crs);
	}
    }

    writeFileSync("cached.json", JSON.stringify(data));
};

for(const [i, courseString] of allCourses.entries()){
    const course = getCourse(courseString);
    if(!course){
...
for(const [i,val] of terms.entries()){
    ...
    if(!set) continue;
    for(const lack of crs.filter(c=>!set.has(c))){
...
for(const {name,values} of terms){
    const foo = new Map();
    console.log(`${name.padStart(10, " ")} - ${values.map(c=>getCourse(c).credits).reduce((a,b)=>a+b) + (foo.get(name) ?? 0)} credits`)
}
console.log(allCourses.map(cs=>getCourse(cs).credits).reduce((a,v)=>a+v)+4+9 + "/120 credits")
